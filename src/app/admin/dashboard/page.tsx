        ...
        <Card extra="p-5">
          ...
          <div className="mt-4 flex flex-col gap-3">
            {!hasAlerts ? (
              ...
              <>
                {lowGreenLots.map(l => (
                  <div key={l.id} className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
                    🌿 {l.lot_name}: {l.remaining_kg} kg left
                  </div>
                ))}
                {channelTotals.map(c => (
                  <div key={c.channel} className="rounded-xl bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
